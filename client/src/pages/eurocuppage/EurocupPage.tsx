...
user_name : string, forecast_points : number, exact : number, difference : number, outcome : number
}

export interface IEuroMatch {
  is_double : boolean, score1 : number, score2 : number, teams : [string, string]
}

export interface IEuroForecasts {
  user_name : string, user_score : [], user_forecast : [{score1: number, score2: number}], forecast_points: number
}
...
export const EurocupPage = () => {
  ...
  <EuroTable data={data.tour.table}/> 
    ...
    <div>
      <ForecastList double_match={doubleMatch} forecasts={data.tour.forecasts}/>
    </div>
    </div>
  : <>No data</>}</>
