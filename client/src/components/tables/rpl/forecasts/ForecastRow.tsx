...
export const ForecastRow: FC<Props> = (props) => {
  return (
    <div className={style.row}>
      ...
      <div>
        <div className={style.forecast_wrapper}>
          {props.el.user_forecast.map((el1, index) => {
            return (
              <div
                key={index}
                className={
                  props.el.user_doubleMatch === index
                    ? style.forecasts_double
                    : style.forecasts
                }
              >
                <span>{el1.score1}</span>
                <span>-</span>
                <span>{el1.score2}</span>
              </div>
            );
          })}
        </div>
        <div className={style.forecast_wrapper}>
          {props.el.user_score.map((el, index) => {
            return (
              <div
                key={index}
                className={
